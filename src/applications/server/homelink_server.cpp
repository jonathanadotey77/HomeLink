...
void handleCLICommand(int commandSocket, const struct sockaddr *sourceAddress,
                      socklen_t sourceAddressLen, const std::string &input) {
  printf("%s\n", input.c_str());
  ...
  int rc = sendto(commandSocket, buffer, sizeof(buffer), 0, sourceAddress,
                  sourceAddressLen);
  if (rc < 0) {
...
void *commandThread(void *) {
  ...
  while (!isStopped) {
    ...
    rc = recvfrom(commandSocket, buffer, sizeof(buffer), 0,
    ...
    if (rc == CLIPacket_SIZE && packetType == e_CLI) {
      ...
      rsaDecrypt(reinterpret_cast<uint8_t *>(data), &dataLen,
                 reinterpret_cast<const uint8_t *>(cliPacket.data),
                 sizeof(cliPacket.data), NULL);
      data[sizeof(data) - 1] = '\0';
      handleCLICommand(
          commandSocket,
          reinterpret_cast<const struct sockaddr *>(&sourceAddress),
          sourceAddressLen, std::string(data));
      ...
      int rc = sendto(commandSocket, buffer, KeyResponsePacket_SIZE, 0,
                      reinterpret_cast<const struct sockaddr *>(&sourceAddress),
                      sourceAddressLen);
      if (rc < 0) {
        fprintf(stderr, "sendto() failed [%d]\n", errno);
  ...
  while (!isStopped) {
    ...
    if (packetType == e_LoginRequest && rc == LoginRequestPacket_SIZE) {
      ...
      if (clientKeys[connectionId].checkTag(tag) &&
          loginSystem.tryLogin(hostId, serviceId,
      ...
      if (verbose) {
        printf(
            "Register request received with {hostId, serviceID} = {%s, %s}\n",
      ...
      LoginStatus status =
          loginSystem.registerUser(hostId, serviceId, password);
      ...
      bool success =
          clientKeys
              .insert({keyRequestPacket.connectionId,
                       KeySet(keyRequestPacket.rsaPublicKey,
                              strlen(keyRequestPacket.rsaPublicKey))})
              .second;
...
void *clientThread(void *a) {
  ...
  *(reinterpret_cast<pthread_t *>(buffer + 1)) = pthread_self();
  int rc = sendto(sd, buffer, sizeof(buffer), 0,
                  reinterpret_cast<const struct sockaddr *>(&commandAddress),
