...
typedef struct LoginStruct {
  LoginStatus status; // 0 if user not found; 1 if login success; 2 if incorrect
                      // password
  const char *password;
} LoginStruct;
...
void LoginSystem::stop() { sqlite3_close(dbHandle); }

LoginStatus LoginSystem::tryLogin(const char *hostId, const char *serviceId,
                                  const char *password) {
  char sql[512] = {0};
  snprintf(sql, sizeof(sql),
           "SELECT password, salt FROM USERS WHERE hostId = '%s' and serviceId "
           "= '%s';",
           hostId, serviceId);
...
LoginStatus LoginSystem::registerUser(const char *hostId, const char *serviceId,
                                      const char *password) {
  ...
  snprintf(sql, sizeof(sql),
           "INSERT INTO USERS (hostId, serviceId, password, salt) VALUES "
           "('%s', '%s', '%s', '%s')",
...
LoginStatus LoginSystem::changePassword(const char *hostId,
  ...
  snprintf(sql, sizeof(sql),
           "UPDATE USERS SET password = '%s', salt = '%s' WHERE hostId = '%s' "
           "and serviceId = '%s';",
